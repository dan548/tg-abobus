...
async def my_send_fn(text: str, criterion: Optional[str]) -> Union[str, dict, float, int]:
    """
    Здесь дерни свой LLM API.
    Мы формируем промпт по заданному шаблону. Модель должна вернуть ТОЛЬКО число 0..100
    (строкой или числом) ИЛИ JSON со score и, по желанию, reason.
    """
...
(оценка от 0 до 100, где 100 — идеально подходит, 0 — совсем не подходит). Ответь только числом, без лишних символов.
Критерии: {crit}
